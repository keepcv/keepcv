...
async function serveStore(
  ...
  const running = await startServer({ port, dataDir, host, auth: setting }).catch(
...
async function main(argv: string[]): Promise<number> {
  ...
  const commands = ["serve", "set-password", "render", "backup", "restore"];
  ...
  return await serveStore(dataDir, chosen.setting, port, values.host ?? DEFAULT_HOST);
}
